<template lang="html">
  <li class="goods-list">
    <router-link :to="{name:'detail',params:{img:img,title:title,price:price}}"  class="goods-list-link">
      <div class="goods-list-pic">
          <img :src="img" alt="">
      </div>
      <div class="goods-list-desc">
        <p class="goods-list-name">{{title}}</p>
        <p class="goods-list-price">{{price}}</p>  <!-- |dTofixed|dCurrency 这里用到了过滤器货币形式和保留两位小数 -->
      </div>
    </router-link>
  </li>
</template>
